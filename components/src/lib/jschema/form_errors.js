...
function setErrorToElement(error, element) {
	...
	if ('params' in error) {
		if (
			...
			setErrorToChildElement(error, error.params.missingProperty, element)
		) {
			return;
		} else if (
			'additionalProperty' in error.params &&
			setErrorToChildElement(error, error.params.additionalProperty, element)
		) {
			return;
		}
	...
	if ('selectedItem' in element) {
		...
		for (let i = 0; i < oneOf.length; i++) {
			...
			if (
				...
				error.instancePath === element.path &&
