...
describe('Admin task-groups page', () => {
	...
	it('Search without filters', async () => {
		...
		expect(mockRequest).toHaveBeenCalledWith(
			new URL('http://localhost:3000/api/admin/v2/task-group?page=1&page_size=50')
		);
	...
	it('Search with filters', async () => {
		...
		expect(mockRequest).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				search:
					'?page=1&page_size=50&user_id=2&user_group_id=1&pkg_name=fractal-tasks-core&origin=pypi&private=false&active=true'
			})
		);
