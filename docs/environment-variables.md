...
- `AUTH_COOKIE_NAME`: the name of the cookie used to store the user session; the default value is `__Host-Http-fastapiusersauth`;
- `AUTH_COOKIE_DOMAIN`: specifies which server can receive the cookie (note: this must **not** include the `http` or `https` URL scheme); Set it in production only if you need to change the default `AUTH_COOKIE_NAME` without using the `__Host-` prefix (see https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Set-Cookie#cookie_prefixes);
