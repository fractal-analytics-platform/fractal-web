...
test.describe('Add single tasks [v2]', () => {
	test.describe.configure({ mode: 'parallel' });

	test('Create non parallel task', async ({ page }) => {
		await openSingleTaskForm(page);
		...
		const task = await getCreatedTaskModalData(page, taskName, 'non_parallel');
	...
	test('Create parallel task', async ({ page }) => {
		await openSingleTaskForm(page);
		const taskName = getRandomName();

		await test.step('Create parallel task v0.0.1', async () => {
			...
			const task = await getCreatedTaskModalData(page, taskName, 'parallel');
			expect(task.name).toEqual(taskName);
		...
		await test.step('Create parallel task v0.0.2', async () => {
			await page.getByRole('combobox', { name: 'Task type' }).selectOption('Parallel');
			await page.getByRole('textbox', { name: 'Task name' }).fill(taskName);
			...
			await page.getByRole('button', { name: /^Create$/ }).click();
			await page.getByText('Task created successfully').waitFor();
			const versionSelector = page.getByRole('row', { name: taskName }).getByRole('combobox');
			await expect(versionSelector).toHaveValue('0.0.2');
			let task = await getCreatedTaskModalData(page, taskName, 'parallel');
			expect(task.name).toEqual(taskName);
			...
			task = await getCreatedTaskModalData(page, taskName, 'parallel');
			expect(task.name).toEqual(taskName);
			expect(task.command_parallel).toEqual('/tmp/test');
			expect(task.args_schema_version).toEqual('pydantic_v2');
		});
	...
	test('Create compound task', async ({ page }) => {
		await openSingleTaskForm(page);
		const taskName = getRandomName();
		...
		const task = await getCreatedTaskModalData(page, taskName, 'compound');
		expect(task.name).toEqual(taskName);
	...
	test('Create task validation', async ({ page }) => {
		await openSingleTaskForm(page);
		const taskName = getRandomName();
		...
		await test.step('Recognize valid pydantic_v2 schema', async () => {
			...
			expect(page.getByRole('combobox', { name: 'Args schema version' })).toHaveValue(
				'pydantic_v2'
			);
		});
 ...
 */
async function openSingleTaskForm(page) {
