<script>
	...
	async function exportTemplate(templateId) {
		...
		if (templateData !== null) {
			...
			const file = new File([JSON.stringify(templateData, null, 2)], name, {
		...
		<div class="card-body">
			<div class="row g-3 align-items-end">
				...
				<div class="col">
					...
					<select id="searchUserEmail" class="form-select form-select-sm" bind:value={userEmail}>
								...
								{#if modalType === 'edit'}
									<td class="col-2">
										...
										<button
											...
											type="button"
											onclick={() => {
												exportTemplate(selectedTemplates[index].template_id);
											}}
											aria-label="Download"
										>
										...
										{#if page.data.userInfo.email == templateGroup.user_email}
											...
											<ConfirmActionButton
												...
												callbackAction={() =>
													handleDeleteTemplate(selectedTemplates[index].template_id)}
											/>
										{/if}
									</td>
								{/if}
...
<TemplateUpdateModal onTemplateSave={searchTemplate} {groups} bind:this={updateTemplateModal} />

<TemplateInfoModal bind:this={infoTemplateModal} />
