...
<Modal id="datasetHistoryOldModal" size="xl" centered={true} scrollable={true}>
	<svelte:fragment slot="header">
		<h5 class="modal-title">Dataset history</h5>
	</svelte:fragment>
	<svelte:fragment slot="body">
		{#if dataset.history && Object.keys(dataset.history).length > 0}
			<div class="accordion" id="accordion-dataset-history-old">
					...
					<div class="accordion-item">
						<h2 class="accordion-header">
							<button
								...
								data-bs-target="#collapse-dataset-history-{index}-old"
								aria-expanded="false"
								aria-controls="collapse-dataset-history-{index}-old"
							>
								Task "{value.workflowtask.task.name}", status "{value.status}"
							</button>
						</h2>
						<div
							id="collapse-dataset-history-{index}-old"
							class="accordion-collapse collapse"
							data-bs-parent="#accordion-dataset-history-old"
						>
