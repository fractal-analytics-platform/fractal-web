<script>
	...
	import { sortDropdownUsers, sortUsers } from '$lib/components/admin/user_utilities';
	...
	async function accountingQuery() {
		errorAlert?.hide();
		searching = true;
		...
		const response = await fetch(
			`/api/admin/v2/accounting?page_size=${pageSize}&current_page=${currentPage}`,
			{
				method: 'POST',
				headers,
				credentials: 'include',
...
{#if accounting}
	<table class="table table-striped mt-3">
		...
		<tbody>
			{#each accounting.accountings as row}
				<tr>
					<td>{row.id}</td>
