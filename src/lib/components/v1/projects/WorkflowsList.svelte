<script>
	...
	function handleWorkflowImported(importedWorkflow) {
		...
		workflows = workflows;
		goto(`/v1/projects/${projectId}/workflows/${importedWorkflow.id}`);
	}
			...
			{#key workflows}
				{#each filteredWorkflows as { id, name }}
					<tr>
						...
						<td>{name}</td>
						<td>
							<a href="/v1/projects/{projectId}/workflows/{id}" class="btn btn-light">
								<i class="bi bi-arrow-up-right-square" />
								Open
							</a>
							<a href="/v1/projects/{projectId}/workflows/{id}/jobs" class="btn btn-light">
