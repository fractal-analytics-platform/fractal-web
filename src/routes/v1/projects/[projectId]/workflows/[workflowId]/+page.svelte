<script>
	...
	import TaskInfoTab from '$lib/components/v1/workflow/TaskInfoTab.svelte';
		...
		if (inputDatasetControl === '') {
			...
			if (response.ok) {
				...
				const jobsUrl = new URL(
					`v1/projects/${project.id}/workflows/${workflow.id}/jobs`,
...
<div class="d-flex justify-content-between align-items-center mb-4">
	<nav aria-label="breadcrumb">
		<ol class="breadcrumb">
			...
			{#if $page.params.projectId}
				<li class="breadcrumb-item" aria-current="page">
					<a href="/v1/projects/{$page.params.projectId}">{project?.name}</a>
				</li>
			{/if}
	...
	<div>
		<a href="/v1/projects/{project?.id}/workflows/{workflow?.id}/jobs" class="btn btn-light">
